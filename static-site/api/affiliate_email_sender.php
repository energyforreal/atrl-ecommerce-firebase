 ...
 * Dedicated email sender for affiliate communications using PHPMailer
...
try {
    ...
    // Initialize PHPMailer for REAL email sending
    ...
    $mail->Password = $cfg['SMTP_PASSWORD'] ?? '8f8c1e4b5c6d7e8f9a0b1c2d3e4f5g6h';
        // Try multiple SMTP configurations based on available extensions
        $smtpConfigured = false;
        
        // Try 1: Standard TLS (requires OpenSSL)
        if (extension_loaded('openssl')) {
            $mail->SMTPSecure = PHPMailer\PHPMailer\PHPMailer::ENCRYPTION_STARTTLS;
            $mail->Port = intval($cfg['SMTP_PORT'] ?? 587);
            $mail->SMTPAutoTLS = true;
            error_log("INFO: Using TLS encryption (OpenSSL available)");
            $smtpConfigured = true;
        } 
        // Try 2: Alternative port that might work without encryption
        else if (isset($cfg['SMTP_ALT_PORT'])) {
            $mail->SMTPSecure = false;
            $mail->SMTPAutoTLS = false;
            $mail->Port = intval($cfg['SMTP_ALT_PORT'] ?? 2525);
            $mail->Host = $cfg['SMTP_ALT_HOST'] ?? $cfg['SMTP_HOST'];
            error_log("INFO: Using alternative SMTP port without encryption");
            $smtpConfigured = true;
        }
        // Try 3: Fallback to unencrypted on standard port (may not work with Brevo)
        else {
            error_log("WARNING: OpenSSL not available, attempting unencrypted connection");
            $mail->SMTPSecure = false;
            $mail->SMTPAutoTLS = false;
            $mail->Port = 25;
            $smtpConfigured = true;
        ...
        if (!$smtpConfigured) {
            throw new Exception('Unable to configure SMTP - no suitable configuration found');
        }
    ...
    switch ($action) {
        case 'welcome':
            $result = sendAffiliateWelcomeEmail($mail, $input);
            break;
            
        case 'commission':
            $result = sendAffiliateCommissionEmail($mail, $input);
            break;
            
        case 'payout':
            $result = sendAffiliatePayoutEmail($mail, $input);
            break;
            
        case 'milestone':
            $result = sendAffiliateMilestoneEmail($mail, $input);
            break;
